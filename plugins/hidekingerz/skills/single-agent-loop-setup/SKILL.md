...
（`/goal` `/loop` `/schedule` は Claude Code の組み込み機能。Codex / opencode から本スキルを使う
場合はこの使い分けは読み飛ばし、「いつ使うか」以降だけ見ればよい）:
...
## セットアップ手順（この順で実施。各項目をハーネスの TODO/プラン機能に積むとよい）
...
```bash
# SKILL_DIR = この SKILL.md が置かれているディレクトリ。
# Claude Code は CLAUDE_SKILL_DIR を自動設定する。Codex / opencode で使う場合は
# このスキルの実際の配置先（例: ~/.agents/skills/single-agent-loop-setup）に置き換える。
SKILL_DIR="${CLAUDE_SKILL_DIR:-$HOME/.agents/skills/single-agent-loop-setup}"

mkdir -p loop
cp "$SKILL_DIR/assets/run.sh"          loop/run.sh
cp "$SKILL_DIR/assets/LOOP_PROMPT.md"  loop/LOOP_PROMPT.md
cp "$SKILL_DIR/assets/VISION.md"       loop/VISION.md
cp "$SKILL_DIR/assets/ARCHITECTURE.md" loop/ARCHITECTURE.md
cp "$SKILL_DIR/assets/RULES.md"        loop/RULES.md
cp "$SKILL_DIR/assets/MEMORY.md"       loop/MEMORY.md
chmod +x loop/run.sh
```
...
## ループを Claude 以外のエージェント CLI で回す（Codex / opencode）

`run.sh` の `AGENT_CMD` は「プロンプトを stdin で受けて標準出力に返すコマンド」なら何でもよいため、
差し替えるだけで他のエージェント CLI でも回せる:

- **Codex CLI**:
  `AGENT_CMD='codex exec --dangerously-bypass-approvals-and-sandbox -'`
  （`-` で stdin からプロンプトを読む。認証は `codex login` 済みか `OPENAI_API_KEY`。
  サンドボックス・承認のバイパスは claude の `--dangerously-skip-permissions` と同じリスクを持つ）
- **opencode**: `opencode run` はメッセージを引数で取るため、stdin を渡す小さなラッパーを挟む:

  ```bash
  printf '#!/bin/sh\nexec opencode run "$(cat)"\n' > loop/agent-opencode.sh
  chmod +x loop/agent-opencode.sh
  # → AGENT_CMD='./loop/agent-opencode.sh'
  ```
...
1. 追加2ファイルを `loop/` に配置:
   ...
   ```bash
   SKILL_DIR="${CLAUDE_SKILL_DIR:-$HOME/.agents/skills/single-agent-loop-setup}"
   cp "$SKILL_DIR/assets/Dockerfile"        loop/Dockerfile
   cp "$SKILL_DIR/assets/run-in-docker.sh"  loop/run-in-docker.sh
...
1. ブラウザ入りイメージと MCP 設定を配置:
   ...
   ```bash
   SKILL_DIR="${CLAUDE_SKILL_DIR:-$HOME/.agents/skills/single-agent-loop-setup}"
   cp "$SKILL_DIR/assets/Dockerfile.frontend"    loop/Dockerfile.frontend
   cp "$SKILL_DIR/assets/chromium-seccomp.json"  loop/chromium-seccomp.json   # 下の run-in-docker.sh コマンドの --security-opt seccomp= が参照
   cp "$SKILL_DIR/assets/mcp.json"               .mcp.json   # ★リポジトリルート（claude -p が読む）
   ```
...
1. 追加3ファイルを配置（プロキシ設定は `loop/proxy/` に置く）:
   ...
   ```bash
   SKILL_DIR="${CLAUDE_SKILL_DIR:-$HOME/.agents/skills/single-agent-loop-setup}"
   cp "$SKILL_DIR/assets/docker-compose.yml"     loop/docker-compose.yml
   cp "$SKILL_DIR/assets/chromium-seccomp.json"  loop/chromium-seccomp.json  # compose が security_opt で参照
   mkdir -p loop/proxy
   cp "$SKILL_DIR/assets/squid.conf"             loop/proxy/squid.conf
   cp "$SKILL_DIR/assets/allowlist.yaml"         loop/proxy/allowlist.yaml
   ```
